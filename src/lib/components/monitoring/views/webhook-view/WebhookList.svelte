	...
	function handleClickOutside(event: MouseEvent) {
		if (activeDropdown) {
			...
			if (
				...
				!target.closest('button[aria-label="Webhook actions menu"]')
			) {
				closeDropdown();
			}
		}
					...
					<Portal>
						{#snippet children()}
							...
							<div
								...
								style="position: fixed; top: {dropdownPositions[activeDropdown!]
									.top}px; left: {dropdownPositions[activeDropdown!].left}px; z-index: 9999;"
							>
								{#if currentWebhook}
									<ul>
										<li>
											<a
												href={`/webhooks/${currentWebhook.id}/edit`}
											...
											>
												<svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
													<path
														...
														stroke-linejoin="round"
														stroke-width="2"
														d="M15 12h.01M12 15h.01M9 12h.01M12 9h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
													/>
												</svg>
												Edit Webhook
											</a>
