	...
	function handleClickOutside(event: MouseEvent) {
		if (activeDropdown) {
			...
			if (
				...
				!target.closest('button[aria-label="URL actions menu"]')
			) {
				closeDropdown();
			}
		}
								...
								<td>
									<div class="flex items-center gap-4">
										...
										<div class="min-w-0 flex-1">
											<div class="truncate text-base font-semibold" title={url.name || url.url}>
								...
								{#if currentUrl}
									<ul>
										<li>
											<button
												onclick={() => {
													toggleStatus(currentUrl);
													closeDropdown();
												}}
												disabled={updating[currentUrl.id || '']}
											>
